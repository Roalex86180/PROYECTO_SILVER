...
// ─── Types ─────────────────────────────────────────────────────────────────
type Payment = {
  id: string; date: string; amount: number; concept: string
  method: string; notes?: string; workerName?: string | null; companyName?: string | null
}
type TeamMember = {
  id: string; name: string; role: string; position?: string
  value?: number; paymentType?: string; payments?: Payment[]
}
type ContractedCompany = {
  id: string; contractId: string; name: string; ein?: string
  contactPerson?: string | null; phone?: string | null
  startDate?: string; endDate?: string; value?: number; paymentType?: string; payments?: Payment[]
}
type ProjectDetail = Project & {
  description?: string; clientContact?: string; startDate?: string; endDate?: string
  budget?: number | null; spent?: number; progress?: number
  team?: TeamMember[]; companies?: ContractedCompany[]; payments?: Payment[]
}
...
// ─── Helpers ────────────────────────────────────────────────────────────────
const fmt = (n?: number | null) => (n !== undefined && n !== null) ? '$' + n.toLocaleString('en-US') : '—'
const fmtDate = (d?: string) => d ? new Date(d).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '—'
const progressColor = (p: number) => p >= 80 ? 'bg-green-500' : p >= 40 ? 'bg-blue-500' : 'bg-yellow-400'

// ─── StatCard ────────────────────────────────────────────────────────────────
function StatCard({ icon, label, value, sub }: { icon: React.ReactNode; label: string; value: string; sub?: string }) {
  return (
    <div className="rounded-xl border border-slate-700/50 px-4 py-3 md:px-5 md:py-4 flex items-center gap-3 md:gap-4"
      style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 55%, #1e3a5f 100%)' }}>
      <div className="w-9 h-9 md:w-10 md:h-10 rounded-lg bg-white/10 border border-white/10 flex items-center justify-center text-blue-300 shrink-0">{icon}</div>
      <div>
...
function ProjectCard({ project, onClick, onStatusChange }: {
  project: Project; onClick: () => void; onStatusChange: (id: string, status: string) => void
}) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 hover:border-blue-300 hover:shadow-md transition-all duration-200 cursor-pointer group flex flex-col" onClick={onClick}>
      <div className="p-4 md:p-5 flex items-start justify-between">
        <div className="flex items-start gap-3 flex-1 min-w-0">
          ...
          <div className="min-w-0">
            ...
            {project.location && <p className="text-xs text-gray-400 flex items-center gap-1 mt-0.5"><MapPin size={10} /> {project.location}</p>}
          </div>
        </div>
        <div className="flex items-center gap-1.5 ml-2 shrink-0" onClick={e => e.stopPropagation()}>
          ...
          <div className="relative" ref={menuRef}>
            <button onClick={() => setMenuOpen(v => !v)} className="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:bg-gray-100 transition-colors">
            ...
            {menuOpen && (
              <div className="absolute right-0 top-8 z-20 bg-white border border-gray-200 rounded-lg shadow-lg py-1 w-40">
                {['active', 'paused', 'completed', 'archived'].filter(s => s !== project.status).map(s => (
                  <button key={s} onClick={() => { onStatusChange(project.id, s); setMenuOpen(false) }}
      ...
      <div className="mt-auto px-4 md:px-5 py-3 border-t border-gray-100 flex items-center justify-between">
        <div className="flex items-center">
          {team.slice(0, 4).map((m, i) => (
            <div key={m.id} title={m.name} style={{ marginLeft: i > 0 ? '-8px' : 0, zIndex: team.length - i }}
          ...
          {team.length > 4 && <div className="w-6 h-6 rounded-full bg-gray-100 border-2 border-white flex items-center justify-center text-gray-500 text-[10px] font-bold -ml-2">+{team.length - 4}</div>}
          {team.length === 0 && <span className="text-xs text-gray-300 flex items-center gap-1"><Users size={11} /> No team</span>}
        </div>
        <div className="flex items-center gap-1 text-xs text-gray-400">
          <Calendar size={11} />
          <span className="hidden sm:inline">{startDate ? fmtDate(startDate) : fmtDate(project.createdAt)}{endDate && <> – {fmtDate(endDate)}</>}</span>
        </div>
...
function ProjectDetailModal({ project, onClose, onExport, onEdit }: {
  project: ProjectDetail; onClose: () => void; onExport: (p: ProjectDetail) => void; onEdit: () => void
}) {
        ...
        <div className="flex items-start justify-between p-4 md:p-6 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {(project.companies ?? []).length > 0 && <p className="text-xs md:text-sm text-gray-400 truncate max-w-[200px] md:max-w-none">{(project.companies ?? []).map(c => c.name).join(", ")}</p>}
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:bg-gray-100 transition-colors"><X size={16} /></button>
          </div>
        </div>
              ...
              {project.budget !== undefined && (
                <div className="bg-gray-50 rounded-lg p-3 md:p-4">
                  ...
                  <div className="grid grid-cols-3 gap-2 md:gap-3 mb-3">
                    ...
                    <div><p className="text-xs text-gray-400">Remaining</p><p className={`text-sm font-bold ${(project.budget ?? 0 - (project.spent ?? 0)) < 0 ? 'text-red-600' : 'text-green-600'}`}>{fmt(project.budget ?? 0 - (project.spent ?? 0))}</p></div>
                  </div>
                  {prog > 0 && (
                    <>
                      ...
                      <div className="h-2 bg-gray-200 rounded-full overflow-hidden"><div className={`h-full rounded-full ${progressColor(prog)}`} style={{ width: `${prog}%` }} /></div>
                    </>
                  )}
                </div>
              )}
          ...
          {tab === 'companies' && (
            <div className="space-y-3">
              {(project.companies ?? []).length === 0 ? (
                <div className="text-center py-10 text-sm text-gray-400"><Building2 size={28} className="mx-auto text-gray-200 mb-2" />No subcontractors assigned</div>
                ...
                <div key={c.contractId} className="border border-gray-100 rounded-xl p-4">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div><p className="text-sm font-bold text-gray-900">{c.name}</p><p className="text-xs text-gray-400">EIN: {c.ein ?? '—'}{c.contactPerson ? ` · ${c.contactPerson}` : ''}</p></div>
                    </div>
                  ...
                  <div className="flex gap-4 mb-3 text-xs text-gray-400"><span>{fmtDate(c.startDate)} – {fmtDate(c.endDate)}</span><span className="capitalize">{c.paymentType}</span></div>
                  {(c.payments ?? []).length > 0 ? (
          ...
          {tab === 'team' && (
            <div className="space-y-2">
              {(project.team ?? []).length === 0 ? (
                <div className="text-center py-10 text-sm text-gray-400"><Users size={28} className="mx-auto text-gray-200 mb-2" />No individual workers assigned</div>
              ) : (project.team ?? []).map(m => (
                <div key={m.id} className="border border-gray-100 rounded-xl p-4">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div><p className="text-sm font-bold text-gray-900">{m.name}</p><p className="text-xs text-gray-400">{m.role}{m.position ? ` · ${m.position}` : ''}</p></div>
                    </div>
          ...
          {tab === 'payments' && (
            <div className="space-y-2">
              {(project.payments ?? []).length === 0 ? (
                <div className="text-center py-10 text-sm text-gray-400"><DollarSign size={28} className="mx-auto text-gray-200 mb-2" />No payments registered</div>
              ) : (
                <>
                  ...
                  {(project.payments ?? []).map(pay => (
                    <div key={pay.id} className="flex items-center justify-between p-3 rounded-lg border border-gray-100 hover:border-blue-200 transition-colors">
                      <div><p className="text-sm font-medium text-gray-800">{pay.concept}</p><p className="text-xs text-gray-400">{fmtDate(pay.date)} · {pay.method}</p></div>
...
// ─── New Project Form ─────────────────────────────────────────────────────────
function NewProjectForm({ onSave, onCancel }: { onSave: (data: any) => Promise<void>; onCancel: () => void }) {
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')
  const [form, setForm] = useState({ name: '', description: '', client: '', clientContact: '', location: '', startDate: '', endDate: '', budget: '', status: 'active' })
  const set = (k: string, v: string) => setForm(p => ({ ...p, [k]: v }))
  const handleSave = async () => {
    if (!form.name.trim()) { setError('Project name is required'); return }
    setSaving(true)
    try { await onSave({ ...form, budget: form.budget ? parseFloat(form.budget) : undefined }) }
    catch (e: any) { setError(e.response?.data?.error || 'Error creating project'); setSaving(false) }
  }
  const field = (label: string, key: string, opts?: { placeholder?: string; type?: string; required?: boolean }) => (
    <div>
      <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1 block">{label} {opts?.required && <span className="text-red-500">*</span>}</label>
      <input type={opts?.type ?? 'text'} className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
        placeholder={opts?.placeholder ?? ''} value={(form as any)[key]} onChange={e => { set(key, e.target.value); setError('') }} />
    </div>
  )
  return (
    <div className="bg-white rounded-xl border border-blue-200 p-4 md:p-5 mb-5 shadow-sm">
      ...
      <div className="mb-3">
        ...
        <textarea className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
          rows={2} placeholder="Brief project description..." value={form.description} onChange={e => set('description', e.target.value)} />
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
        ...
        <div>
          ...
          <select className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" value={form.status} onChange={e => set('status', e.target.value)}>
...
function EditProjectModal({ project, onClose, onSaved }: {
  project: ProjectDetail; onClose: () => void; onSaved: (updated: ProjectDetail) => void
}) {
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')
  const [form, setForm] = useState({
    name: project.name || '', description: project.description || '',
    clientContact: project.clientContact || '', location: project.location || '',
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      const payload: any = { name: form.name, description: form.description || undefined, clientContact: form.clientContact || undefined, location: form.location || undefined, status: form.status, startDate: form.startDate || undefined, endDate: form.endDate || undefined, budget: form.budget ? parseFloat(form.budget) : undefined }
      await projectService.update(project.id, payload)
      onSaved({ ...project, ...payload })
    } catch (e: any) { setError(e.response?.data?.error || 'Error updating project'); setSaving(false) }
  }
  const field = (label: string, key: string, opts?: { placeholder?: string; type?: string; required?: boolean }) => (
    <div>
      <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1 block">{label} {opts?.required && <span className="text-red-500">*</span>}</label>
      <input type={opts?.type ?? 'text'} className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
        placeholder={opts?.placeholder ?? ''} value={(form as any)[key]} onChange={e => set(key, e.target.value)} />
    </div>
  )
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-lg bg-blue-50 flex items-center justify-center"><FolderKanban size={16} className="text-blue-600" /></div>
            <div><h2 className="text-sm font-bold text-gray-900">Edit Project</h2><p className="text-xs text-gray-400">{project.name}</p></div>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:bg-gray-100"><X size={16} /></button>
        </div>
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="col-span-2">
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1 block">Description</label>
              <textarea className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none" rows={3} placeholder="Brief project description..." value={form.description} onChange={e => set('description', e.target.value)} />
            </div>
            <div className="col-span-2">
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1 block">Status</label>
              <select className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" value={form.status} onChange={e => set('status', e.target.value)}>
...
function AnalyticsView({ projects }: { projects: Project[] }) {
  ...
  const withMetrics = completed.map(p => {
    const budget = p.budget ?? 0; const spent = p.spent ?? 0; const profit = budget - spent
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div className="bg-white rounded-xl border border-gray-200 p-4 md:p-5">
          ...
          <div className="space-y-3">
            {byProfit.map((p, i) => (
              <div key={p.id} className={`rounded-lg px-4 py-3 flex items-center justify-between ${i === 0 ? 'bg-yellow-50 border border-yellow-100' : i === 1 ? 'bg-gray-50 border border-gray-100' : i === 2 ? 'bg-orange-50 border border-orange-100' : 'bg-white border border-gray-100'}`}>
                <div className="flex items-center gap-3 min-w-0">
                  <span className="text-lg shrink-0">{medal(i)}</span>
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-gray-800 truncate">{p.name}</p>
                    <p className={`text-xs font-bold mt-0.5 ${p.profit >= 0 ? 'text-green-600' : 'text-red-500'}`}>{p.profit >= 0 ? '+' : ''}{fmt(p.profit)}</p>
                  </div>
                </div>
                <div className="text-right shrink-0 ml-3"><p className="text-xs text-gray-400">Budget</p><p className="text-xs font-semibold text-gray-600">{fmt(p.budget)}</p></div>
              </div>
            ))}
          </div>
        </div>
            ...
            {byMargin.map((p, i) => (
              <div key={p.id}>
                ...
                <div className="h-2.5 bg-gray-100 rounded-full overflow-hidden">
                  <div className={`h-full rounded-full transition-all duration-500 ${marginBg(p.margin)}`} style={{ width: `${Math.max(0, Math.min(100, p.margin))}%` }} />
                </div>
              </div>
            ))}
          </div>
            ...
            <div key={p.id}>
              <div className="flex items-center justify-between mb-1 flex-wrap gap-2">
                ...
                <div className="flex items-center gap-3 md:gap-5 text-xs shrink-0">
                  ...
                  <span className={`font-bold ${p.profit >= 0 ? 'text-green-600' : 'text-red-500'}`}>{p.profit >= 0 ? '+' : ''}{fmt(p.profit)}</span>
                </div>
              </div>
              <div className="h-2.5 bg-gray-100 rounded-full overflow-hidden flex">
                <div className="h-full bg-blue-400 transition-all duration-500" style={{ width: `${p.budget > 0 ? Math.min(100, (p.spent / p.budget) * 100) : 0}%` }} />
                <div className="h-full bg-green-400 transition-all duration-500" style={{ width: `${p.budget > 0 ? Math.max(0, ((p.budget - p.spent) / p.budget) * 100) : 0}%` }} />
              </div>
...
export default function Projects() {
  ...
  useEffect(() => { projectService.getAll().then(p => { setProjects(p); setLoading(false) }) }, [])

  const filtered = projects.filter(p => {
    const matchSearch = p.name.toLowerCase().includes(search.toLowerCase()) || ((p as any).client || '').toLowerCase().includes(search.toLowerCase()) || (p.location || '').toLowerCase().includes(search.toLowerCase())
  ...
  const handleCreate = async (data: any) => { const np = await projectService.create(data); setProjects(p => [np, ...p]); setShowForm(false) }
  ...
  const handleCardClick = async (project: Project) => {
    ...
    try { const detail = await projectService.getById(project.id); setSelected(detail as ProjectDetail) }
    catch { setSelected(project as ProjectDetail) }
    finally { setLoadingDetail(false) }
  }
  const handleProjectSaved = (updated: ProjectDetail) => {
    setProjects(prev => prev.map(p => p.id === updated.id ? { ...p, ...updated } : p))
    setSelected(updated); setEditingProject(null)
  }
  ...
  return (
    <div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex gap-1 bg-gray-100 rounded-lg p-1">
            <button onClick={() => setView('projects')}
              className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${view === 'projects' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
              <FolderKanban size={13} /> Projects
            </button>
            <button onClick={() => setView('analytics')}
              className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${view === 'analytics' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
              <BarChart2 size={13} /> Analytics
            </button>
          </div>
          {view === 'projects' && <Button onClick={() => setShowForm(v => !v)}><Plus size={15} /> New Project</Button>}
        </div>
      </div>
        ...
        <>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-5">
            <div className="relative flex-1">
              ...
              <input className="w-full pl-9 pr-4 py-2 text-sm border border-gray-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="Search by name, client or location..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
            {/* Filter — scrollable on mobile */}
            <div className="flex gap-1 bg-gray-100 rounded-lg p-1 overflow-x-auto">
              {(['all', 'active', 'paused', 'completed', 'archived'] as const).map(s => (
                <button key={s} onClick={() => setFilter(s)}
                  className={`px-3 py-1.5 text-xs font-medium rounded-md capitalize transition-colors shrink-0 ${filter === s ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
          ...
          {loading ? (
            ...
            <div className="text-center py-16 bg-white rounded-xl border border-dashed border-gray-200">
              ...
              <p className="text-sm text-gray-400">{search || filter !== 'all' ? 'No projects match your filters' : 'No projects yet'}</p>
              {!search && filter === 'all' && <button onClick={() => setShowForm(true)} className="mt-3 text-sm text-blue-600 hover:underline">Create first project</button>}
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filtered.map(p => <ProjectCard key={p.id} project={p} onClick={() => handleCardClick(p)} onStatusChange={handleStatusChange} />)}
            </div>
          )}
        </>
      )}
      ...
      {editingProject && <EditProjectModal project={editingProject} onClose={() => setEditingProject(null)} onSaved={handleProjectSaved} />}
      {selected && <ProjectDetailModal project={selected} onClose={() => setSelected(null)} onExport={exportProjectExcel} onEdit={() => setEditingProject(selected)} />}
    </div>
  )
}