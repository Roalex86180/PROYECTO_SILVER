...
import { Plus, FileText, CreditCard, Search, User, Building2, ChevronRight, Download, Pencil, X, Paperclip, ArrowLeft } from 'lucide-react'
  ...
  const handleSave = async () => {
    ...
    try {
      const updated = await contractService.update(contract.id, {
        ...
        paymentType: form.paymentType, value: parseFloat(form.value),
      })
...
function EditCompanyModal({ company, onClose, onSaved }: {
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try { const updated = await companyService.update(company.id, form); onSaved(updated) }
    catch (err: any) { setErrors({ submit: err.response?.data?.error || 'Error updating company' }); setSaving(false) }
  }
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-lg bg-gray-100 flex items-center justify-center"><Building2 size={16} className="text-gray-500" /></div>
            <div><h2 className="text-sm font-bold text-gray-900">Edit Company</h2><p className="text-xs text-gray-400">{company.name}</p></div>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:bg-gray-100"><X size={16} /></button>
        </div>
        <div className="flex-1 overflow-y-auto px-6 py-5">
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2"><FormField label="Company Name" required error={errors.name}><Input value={form.name} onChange={e => set('name', e.target.value)} error={!!errors.name} /></FormField></div>
            <FormField label="EIN" required error={errors.ein}><Input placeholder="12-3456789" value={form.ein} onChange={e => set('ein', e.target.value)} error={!!errors.ein} /></FormField>
            <FormField label="Contact Person"><Input placeholder="John Smith" value={form.contactPerson} onChange={e => set('contactPerson', e.target.value)} /></FormField>
            <FormField label="Phone"><Input placeholder="(555) 000-0000" value={form.phone} onChange={e => set('phone', e.target.value)} /></FormField>
            <FormField label="Email"><Input type="email" placeholder="contact@company.com" value={form.email} onChange={e => set('email', e.target.value)} /></FormField>
            <FormField label="Address"><Input placeholder="123 Main St" value={form.address} onChange={e => set('address', e.target.value)} /></FormField>
            <FormField label="State"><Select options={US_STATES} placeholder="Select state..." value={form.state} onChange={e => set('state', e.target.value)} /></FormField>
            <div className="col-span-2"><FormField label="Notes"><textarea className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none" rows={2} placeholder="Optional notes..." value={form.notes} onChange={e => set('notes', e.target.value)} /></FormField></div>
          </div>
...
function EditWorkerModal({ worker, companies, onClose, onSaved }: {
  worker: WorkerDetail; companies: Company[]
  onClose: () => void; onSaved: (updated: Worker) => void
}) {
  const [saving, setSaving] = useState(false)
  const [errors, setErrors] = useState<Record<string, string>>({})
  const [form, setForm] = useState({
    name: worker.name || '', ssn: worker.ssn || '', ein: worker.ein || '',
    phone: worker.phone || '', email: worker.email || '', address: worker.address || '',
    state: worker.state || '', workAuthorization: worker.workAuthorization || '',
    role: worker.role || '', type: worker.type || '', companyId: worker.companyId || '',
    emergencyContact: worker.emergencyContact || '', emergencyPhone: worker.emergencyPhone || '',
  })
  const set = (k: string, v: string) => { setForm(p => ({ ...p, [k]: v })); setErrors(p => ({ ...p, [k]: '' })) }
  const validate = () => {
    const e: Record<string, string> = {}
    if (!form.name) e.name = 'Required'; if (!form.ssn) e.ssn = 'Required'
    if (!form.role) e.role = 'Required'; if (!form.type) e.type = 'Required'
  ...
  const handleSave = async () => {
    ...
    try {
      const updated = await workerService.update(worker.id, {
        name: form.name, ssn: form.ssn, ein: form.ein || undefined,
        phone: form.phone || undefined, email: form.email || undefined,
        address: form.address || undefined, state: form.state || undefined,
        workAuthorization: form.workAuthorization, role: form.role, type: form.type,
        companyId: form.companyId || undefined, emergencyContact: form.emergencyContact || undefined,
        emergencyPhone: form.emergencyPhone || undefined,
      })
      onSaved(updated)
    } catch (e: any) { setErrors({ submit: e.response?.data?.error || 'Error updating worker' }); setSaving(false) }
  }
  const companyOptions = [{ value: '', label: 'None (Silver Star direct employee)' }, ...companies.map(c => ({ value: c.id, label: `${c.name} (EIN: ${c.ein})` }))]
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-blue-600 flex items-center justify-center text-white font-bold">{worker.name[0]}</div>
            <div><h2 className="text-sm font-bold text-gray-900">Edit Worker</h2><p className="text-xs text-gray-400">{worker.name}</p></div>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:bg-gray-100"><X size={16} /></button>
        </div>
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2"><FormField label="Full Name" required error={errors.name}><Input value={form.name} onChange={e => set('name', e.target.value)} error={!!errors.name} /></FormField></div>
              <FormField label="SSN" required error={errors.ssn}><Input placeholder="XXX-XX-XXXX" value={form.ssn} onChange={e => set('ssn', e.target.value)} error={!!errors.ssn} maxLength={11} /></FormField>
              <FormField label="EIN (if applicable)"><Input placeholder="XX-XXXXXXX" value={form.ein} onChange={e => set('ein', e.target.value)} /></FormField>
              <FormField label="Phone"><Input placeholder="+1 (555) 000-0000" value={form.phone} onChange={e => set('phone', e.target.value)} /></FormField>
              <FormField label="Email"><Input type="email" placeholder="john@email.com" value={form.email} onChange={e => set('email', e.target.value)} /></FormField>
              <div className="col-span-2"><FormField label="Address"><Input placeholder="123 Main St, City" value={form.address} onChange={e => set('address', e.target.value)} /></FormField></div>
              <FormField label="State"><Select options={US_STATES} placeholder="Select state..." value={form.state} onChange={e => set('state', e.target.value)} /></FormField>
            </div>
          </div>
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <FormField label="Role" required error={errors.role}><Select options={ROLES} placeholder="Select role..." value={form.role} onChange={e => set('role', e.target.value)} error={!!errors.role} /></FormField>
              <FormField label="Worker Type" required error={errors.type}><Select options={WORKER_TYPES} placeholder="Select type..." value={form.type} onChange={e => set('type', e.target.value)} error={!!errors.type} /></FormField>
              <div className="col-span-2"><FormField label="Work Authorization" required error={errors.workAuthorization}><Select options={WORK_AUTH} placeholder="Select authorization..." value={form.workAuthorization} onChange={e => set('workAuthorization', e.target.value)} error={!!errors.workAuthorization} /></FormField></div>
              <div className="col-span-2"><FormField label="Company (if applicable)"><Select options={companyOptions} placeholder="Select company..." value={form.companyId} onChange={e => set('companyId', e.target.value)} /></FormField></div>
            </div>
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-3">Emergency Contact</p>
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2"><FormField label="Contact Name"><Input placeholder="Jane Smith" value={form.emergencyContact} onChange={e => set('emergencyContact', e.target.value)} /></FormField></div>
              <FormField label="Contact Phone"><Input placeholder="+1 (555) 000-0000" value={form.emergencyPhone} onChange={e => set('emergencyPhone', e.target.value)} /></FormField>
            </div>
          </div>
...
export default function HumanResources() {
  ...
  // Mobile: track if detail panel is open
  ...
  useEffect(() => {
    if (search.length > 0) { setSuggestions(workers.filter(w => w.name.toLowerCase().includes(search.toLowerCase()))); setShowSug(true) }
  ...
  const handleGlobalExport = async () => {
    ...
    try { const allDetails = await Promise.all(workers.map(w => workerService.getById(w.id))); setExportData(allDetails as WorkerDetail[]); setExportCompanyData([]); setShowExport(true) }
    catch (error) { console.error('Error preparing global export', error) }
  ...
  const handleGlobalCompanyExport = () => {
    ...
    setExportData([]); setShowExport(true)
  }
  ...
  const handleContractSaved = (updated: ContractFull) => {
    ...
    setSelected(prev => prev ? { ...prev, contracts: (prev.contracts || []).map(c => c.id === updated.id ? { ...c, ...updated, value: String(updated.value) } : c) } : prev)
    setEditingContract(null)
  }

  const displayedWorkers = search ? workers.filter(w => w.name.toLowerCase().includes(search.toLowerCase())) : workers
  const totalPaid = (contracts: ContractLocal[]) => contracts.flatMap(c => c.payments).reduce((sum, p) => sum + Number(p.amount), 0)
    ...
    <div className="flex items-center justify-between px-4 py-2.5">
      ...
      <div className="flex items-center gap-3">
        <div className="text-right">
          <p className="text-sm font-semibold text-green-600">${Number(payment.amount).toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
          <p className="text-xs text-gray-400">{payment.method} · {new Date(payment.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</p>
        </div>
        {payment.receiptUrl && (
          <button onClick={() => { setZoom(1); setReceiptModal(payment.receiptUrl) }}
            className="w-7 h-7 rounded-lg flex items-center justify-center text-blue-400 hover:bg-blue-50 hover:text-blue-600 transition-colors" title="View receipt">
            <Paperclip size={13} />
          </button>
        )}
      </div>
  ...
  return (
    <div>
      ...
      {tab === 'workers' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <div className={`md:col-span-2 ${mobileDetailOpen && mobileDetailType === 'worker' ? 'block' : 'hidden md:block'}`}>
            {loadingDetail ? (
              <div className="bg-white rounded-xl border border-gray-200 flex items-center justify-center h-64"><p className="text-sm text-gray-400">Loading...</p></div>
            ) : selected ? (
                ...
                <div className="p-4 md:p-5 border-b border-gray-100">
                  ...
                  <div className="flex flex-wrap gap-4 mt-4 pt-4 border-t border-gray-50">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-gray-700">{selected.companyId ? (companies.find(c => c.id === selected.companyId)?.name ?? 'Unknown') : 'Silver Star Direct'}</p>
                    </div>
                      ...
                      {selected.contracts.map(contract => (
                        <div key={contract.id} className="border border-gray-100 rounded-lg overflow-hidden">
                          ...
                          {contract.payments.length > 0
                            ? <div className="divide-y divide-gray-50">{contract.payments.map(p => <PaymentRow key={p.id} payment={p} />)}</div>
      ...
      {/* Modals */}
      {editingContract && <EditContractModal contract={editingContract} onClose={() => setEditingContract(null)} onSaved={handleContractSaved} />}
      {editingCompany && <EditCompanyModal company={editingCompany} onClose={() => setEditingCompany(null)} onSaved={handleCompanySaved} />}
      {editingWorker && <EditWorkerModal worker={editingWorker} companies={companies} onClose={() => setEditingWorker(null)} onSaved={handleWorkerSaved} />}
    </div>
  )
}