...
import { Plus, FileText, CreditCard, Search, User, Building2, ChevronRight, Download, Pencil, X } from 'lucide-react'
    ...
    <div>
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          >
            <Download size={15} /> {isPreparingExport ? 'Preparing...' : 'Export All (Dates)'}
          </button>
          <button
      ...
      {showExport && <ExportModal onClose={() => setShowExport(false)} workers={exportData} />}
                  ...
                  <div className="flex gap-6 mt-4 pt-4 border-t border-gray-50">
                    ...
                    <div className="ml-auto text-right">
                      ...
                      <div className="flex items-center gap-2 mt-2 justify-end">
                        ...
                        <button onClick={() => { setExportData([selected]); setShowExport(true) }}
            ...
            <div className="grid grid-cols-3 gap-4">
              {companies.map(c => (
                <div key={c.id} className="bg-white rounded-xl border border-gray-200 p-5">
                  ...
                  {c.contactPerson && <p className="text-xs text-gray-500 mb-1">👤 {c.contactPerson}</p>}
                  {c.phone        && <p className="text-xs text-gray-400 mb-1">📞 {c.phone}</p>}
                  {c.email        && <p className="text-xs text-gray-400 mb-1">✉ {c.email}</p>}
                  {c.state        && <p className="text-xs text-gray-400">📍 {c.state}</p>}
                  {c.notes        && <p className="text-xs text-gray-300 mt-2 italic truncate">{c.notes}</p>}
                </div>
              ))}
            </div>
          )}
                ...
                <div key={c.id} className="bg-white rounded-xl border border-gray-200 px-5 py-4 flex items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-3 w-48 shrink-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-gray-900 truncate">
                        {c.worker?.name ?? c.company?.name ?? '—'}
                      </p>
                      <p className="text-xs text-gray-400">
                        {c.worker?.role ?? `EIN: ${c.company?.ein ?? '—'}`}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm font-medium text-gray-700 truncate">
                      {(c as any).project?.name ?? c.projectId}
                    </p>
                  </div>
